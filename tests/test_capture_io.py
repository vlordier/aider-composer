...
def test_get_captured_write_files(capture_io):
    ...
    assert write_files == {"file1.txt": "content1", "file2.txt": "content2"}
    assert capture_io.write_files == {}  # Should be empty after getting


def test_confirm_ask(capture_io):
    assert capture_io.confirm_ask("Create new file test.txt") is True
    assert capture_io.confirm_ask("Other question") is False
    assert capture_io.confirm_ask("Create new file", subject="test.txt") is True
