...
def chat_manager(mock_coder):
    with patch('server.chat_session_manager.Coder') as MockCoder, \
         patch('server.chat_session_manager.Model') as MockModel:
        MockCoder.create.return_value = mock_coder
...
def test_chat_basic_message(chat_manager, mock_coder):
    ...
    assert len(chunks) == 3  # 2 data chunks + 1 end chunk
...
def test_chat_with_error(chat_manager, mock_coder):
    ...
    chunks = list(chat_manager.chat(data))
    assert len(chunks) == 2  # 1 error chunk + 1 end chunk
